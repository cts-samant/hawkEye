...
flowFile = session.write(flowFile, {inputStream, outputStream ->
    try {
        def fname = flowFile.getAttribute('filename').toString()
        ...
        def query = "INSERT INTO public.dms_common_error_table (error_code, error_type, error_description, error_context, interface, reference, createts, modifyts, create_user_id, modify_user_id)\n" +
                "VALUES ('DMSEXCEP004', 'Data Processing Failed', 'Data Processing Failed', 'SoftwareVersion', 'dms_softwareVersionInterface', '"+fname+"', '"+date_time+"', '"+date_time+"', 'Nifi', 'Nifi')"
        log.info(query)
